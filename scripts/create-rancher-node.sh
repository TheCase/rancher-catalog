...
echo "Create VM..."
cmd="pvesh create /nodes/${NODE}/qemu -name=${TYPE}${VMID} -vmid ${VMID} -memory 8192 -sockets 1 -cores 4 -net0 e1000,bridge=vmbr0,macaddr=${MACPRE}:${MSUF} -ide0=local-lvm:vm-${VMID}-disk-0"
echo $cmd
ssh ${NODE} ${cmd} 2> /dev/null
